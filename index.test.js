...
test('addBinary correctly adds two binary arrays of the same length', () => {
  ...
  expect(addBinary(eight, two)).toEqual([0, 1, 0, 0, 1]);
  expect(addBinary(eight, three)).toEqual([0, 1, 0, 1, 1]);
  expect(addBinary(eight, eight)).toEqual([1, 0, 0, 0, 0]);
  expect(addBinary(fifteen, fifteen)).toEqual([1, 1, 1, 1, 0]);
});
